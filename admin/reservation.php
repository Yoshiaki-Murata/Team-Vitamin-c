      ...
      <tbody>
        <?php foreach ($reservations as $row): ?>
          <tr>
            ...
            <td><?php echo $row['reserve_class'] ? h($row['reserve_class']) : '未定' ?></td>
            <td><?php echo $row['reserve_consultant'] ? h($row['reserve_consultant']) : '未定' ?></td>
            <td><?php echo $row['reserve_method'] ? h($row['reserve_method']) : '-' ?></td>
              ...
              <div class="mb-3">
                ...
                <select name="student_id" id="studentSelect" class="form-select">
                  ...
                  <?php foreach ($students as $student): ?>
                    <option value="<?php echo h($student['id']) ?>"
                      data-class-id="<?php echo h($student['class_id']) ?>">
    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      document.querySelectorAll('.add-btn').forEach(btn => {
        btn.addEventListener('click', () => {
          document.getElementById('add-slot-id').value =
